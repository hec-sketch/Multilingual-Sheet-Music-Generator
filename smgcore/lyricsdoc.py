...
HEADING_START = re.compile(rf"^\s*\(?\s*{SECTION_WORD}(?![^\W\d_])", re.I)
...
# The trailing ')' matches a heading written as one whole parenthesised aside,
# e.g. '(Pre-Ch2)' - a translator's way of marking a section as a repeat of one
...
def heading_labels(row: str) -> list[str] | None:
    ...
    if not match or not HEADING_TAIL.match(tail):
        return None
    word = match.group(0).strip().lstrip("(").strip()
...
def parse_lyrics_document(data: bytes, english_vocabulary=None) -> LyricsDoc:
    ...
    (the same test ``layout.split_in_half`` uses for a two-language grid),
...
def strip_english_lines(lyrics: LyricsDoc, english_vocabulary) -> LyricsDoc:
    ...
    ``layout.split_in_half`` uses to tell the two halves of a two-language
...
def prefer_lyrics_sheet(lyrics: LyricsDoc, layout_doc, score_doc) -> bool:
    ...
    """
    from .layout import split_in_half, to_editable
    ...
    # A grid in two halves - the English above, the translation below - is a
    # better source than the same file read as prose.
    english, translated = split_in_half(
        lines, layout_doc.page_count, score_doc.sung_words()
    )
